...
def login():
    if user_name.get() == "" or password.get() == "":
        ...
        try:
            ...
            cur.execute("select * from patients where user_name=%s and password = %s",
...
def signup():
    ...
    def action():
        if first_name.get() == "" or last_name.get() == "" or age.get() == "" or city.get() == "" or add.get() == "" or user_name.get() == "" or password.get() == "" or very_pass.get() == "":
            ...
            try:
                con = pymysql.connect(host="localhost", user="root", password="", database="register")
                cur = con.cursor()
                cur.execute("select * from patients where user_name=%s", user_name.get())
                row = cur.fetchone()
                if row != None:
    ...
    winsignup = Tk()
    winsignup.title("Health consulting")
